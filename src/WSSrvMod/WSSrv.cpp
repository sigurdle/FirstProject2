// WSSrv.cpp : Implementation of CWSSrv
#include "stdafx.h"
#include "WSSrvMod.h"
#include "WSSrv.h"
...
void CWSSrv::CreateTable_websettings(ADOX::_CatalogPtr catalog)
{
...
void CWSSrv::CreateTable_files(ADOX::_CatalogPtr catalog)
{
	ADOX::_TablePtr table("ADOX.Table");
...
void CWSSrv::CreateTable_files_data(ADOX::_CatalogPtr catalog)
{
	ADOX::_TablePtr table("ADOX.Table");
...
void CWSSrv::CreateTable_links(ADOX::_CatalogPtr catalog)
{
	ADOX::_TablePtr table("ADOX.Table");
...
void CWSSrv::CreateTable_tasks(ADOX::_CatalogPtr catalog)
{
	ADOX::_TablePtr table("ADOX.Table");
...
void CWSSrv::GetHomepageFileIdFromDB()
{
	try
	{
	...
	{
		MessageBox(NULL, "Error GetHomepageFileIdFromDB()", "WebEditorServer", MB_OK);
		DbError(e);
	}
}

STDMETHODIMP CWSSrv::SetHomepageFileId(unsigned long file_id)
{
	try
	{
		ADODB::_RecordsetPtr recordset("ADODB.RecordSet");
	...
	{
		MessageBox(NULL, "Error SetHomepageFileId()", "WebEditorServer", MB_OK);
		DbError(e);
	}
...
STDMETHODIMP CWSSrv::GetHomepageFileId(unsigned long *file_id)
{
...
STDMETHODIMP CWSSrv::LoadDocument(BSTR pathName, VARIANT_BOOL *success)
{
...
//		m_rootPath = rootpath;
		...
		if (m_siteDataConnection == NULL)
		{
			...
			{
				//MessageBox(NULL, "Error WebSite Load()", "WebEditorServer", MB_OK);
				DbError(e);
			}
		}
...
STDMETHODIMP CWSSrv::NewDocument(BSTR pathName, BSTR siteName)
{
	try
	{
		TCHAR fullpath[512];
...

		{
			...
			_makepath(xmlfilepath, NULL, fullpath, _bstr_t(siteName), _T(".site"));
...
STDMETHODIMP CWSSrv::GetDBConnection(IDispatch **pVal)
{
...
STDMETHODIMP CWSSrv::get_rootPath(BSTR *pVal)
{
	ATLASSERT(0);
/*
	if (pVal == NULL) return E_POINTER;
...
STDMETHODIMP CWSSrv::SaveArrayAsFile(unsigned long folder_id, BSTR fileName, SAFEARRAY* dataArray, BSTR user, DWORD* pfile_id, VARIANT_BOOL* pSuccess)
{
...
STDMETHODIMP CWSSrv::OpenFileAsArray(unsigned long file_id, SAFEARRAY** dataArray)
{
	...
	*dataArray = NULL;
	...
	{
		MessageBox(NULL, "Error OpenFileAsArray()", "WebEditorServer", MB_OK);
		DbError(e);
	}
...
_bstr_t CWSSrv::GetFolderDisplayPathRecursive(DWORD folder_id, WCHAR* pathDelimiter)
{
	_bstr_t name = L"";
...
STDMETHODIMP CWSSrv::GetFolderDisplayPath(DWORD folder_id, BSTR *displayName)
{
...
STDMETHODIMP CWSSrv::GetParentFolderId(DWORD folder_id, DWORD *pVal)
{
	if (pVal == NULL) return E_POINTER;
...
DWORD CWSSrv::AddNewFile(DWORD parent_id, long type, BSTR filename, DWORD file_size, double file_date)
{
...
STDMETHODIMP CWSSrv::FindOrCreateFolder(DWORD parent_id, BSTR name, BSTR user, DWORD *pfolder_id)
{
...
STDMETHODIMP CWSSrv::CreateFolder(DWORD parent_id, BSTR name, BSTR user, DWORD *pfolder_id)
{
	if (pfolder_id == NULL) return E_POINTER;
	*pfolder_id = 0L;
...
void CWSSrv::RemoveFileFromDatabase(DWORD parent_id, DWORD id)
{
	WCHAR sql[256];
	_variant_t va;
...
STDMETHODIMP CWSSrv::DeleteFilePermanently(unsigned long file_id, VARIANT_BOOL *pSuccess)
{
	if (pSuccess == NULL) return E_POINTER;
...
STDMETHODIMP CWSSrv::RenameFile(DWORD file_id, BSTR name, VARIANT_BOOL *pSuccess)
{
	if (pSuccess == NULL) return E_POINTER;
