...
AExpression* Parser::ParseChoice()
{
	...
	{
		lex();
		left = new AChoice(m_loc, left, ParseLogicalANDExpression());
	}

	return left;
}

ATerminalList* Parser::ParseItem()
{
	ATerminalList* dlist = new ATerminalList;
	while (!eof())
	{
		ARule* decl = ParseTerminal();
...
ADeclaration* Parser::ParseRule()
{
	String name = readidentifier();
	EatChar(':');
	TerminalList* tlist = ParseTerminals();
	EatChar(';');
	return new ARule(m_loc, name, terminals);
}

ARuleList* Parser::ParseGrammar()
{
	ARuleList* dlist = new ARuleList;
	while (!eof())
	{
		ARule* decl = ParseRule();
...
// static
ADeclarationList* Parser::GrammarFromFile(StringIn filename)
{
	IO::FileStream file(filename, IO::FileMode_Open, IO::FileAccess_Read);
	return ParseGrammar(&file);
}
