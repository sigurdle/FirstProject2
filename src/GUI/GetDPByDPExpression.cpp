...
void GetDPByDPExpression::OnChildChanged(Expressive::Expression* exp)
{
	...
	TraceCallback(&context, Expressive::ExpTrace(), nullptr);
...
int GetDPByDPExpression::SetInput(IBinding* caller, const Variant& value)
{
	if (m_setting == false)
	{
		m_setting = true;
	...
	//	m_parent->OnChildChanged(this);
		m_ownerBinding->m_exp->TraceCallback(&context, Expressive::ExpTrace(), nullptr);
...
Variant GetDPByDPExpression::Evaluate(Expressive::EvalContext* context)
{
	if (m_object == nullptr)
	{
		ASSERT(0);
		raise(Exception("(null).GetDPPropertyByDPExpression"));
	}
...
#endif
	{
		return depObj->GetProperty(m_dp)->GetComputedValue();
	}
}

bool GetDPByDPExpression::Set(Expressive::EvalContext* context, const Variant& value)
{
	ASSERT(!m_setting);
