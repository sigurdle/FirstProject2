...
public:
    CTOR FOURCCMap();
    CTOR FOURCCMap(uint32 Fourcc);
    CTOR FOURCCMap(const GUID *);

    uint32 GetFOURCC(void);
...
FOURCCMap::InitGUID() {
    Data2 = GUID_Data2;
    Data3 = GUID_Data3;
    ((uint32 *)Data4)[0] = GUID_Data4_1;
    ((uint32 *)Data4)[1] = GUID_Data4_2;
}
...
inline
FOURCCMap::FOURCCMap(uint32 fourcc)
{
    InitGUID();
    SetFOURCC(fourcc);
}
...
inline void
FOURCCMap::SetFOURCC(uint32 fourcc)
{
    Data1 = fourcc;
}
