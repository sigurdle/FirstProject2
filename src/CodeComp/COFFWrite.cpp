...
// Write section data
	...
	if (true)	// .debug$S
	{
		...
		memcpy((char*)pSectionHeader->Name, ".debug$S", 8);
...

	{
		...
		for (size_t i = 0; i < globals.size(); ++i)
		{
			...
			memcpy((char*)pSectionHeader->Name, ".text\0\0", 8);
		...
		//	ASSERT(0);
...

			{
				...
				while (il)
				{
					...
					else if (instr->kind == AS_instr::I_FUN)
					{
						...
						*/
...

						{
							...
							memcpy((char*)efsym.N.ShortName, ".ef\0\0\0\0", 8);
					...
					{
						IMAGE_SYMBOL symbol = {0};
						memcpy((char*)symbol.N.ShortName, ".file\0\0", 8);
						...
						for (size_t i = 0; i < symbol.NumberOfAuxSymbols; ++i)
						{
