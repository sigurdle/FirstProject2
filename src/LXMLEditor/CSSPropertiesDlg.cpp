...
CComBSTR MakeRGBString(int colorFormat, double red, double green, double blue, double alpha);

class CPopupDialog :
	public CWindowImpl<CPopupDialog>
{
public:
//	DECLARE_WND_CLASS_EX("PopupSlider", CS_DBLCLKS, COLOR_BTNFACE)
	...
	int Popup(HWND hWnd, CPoint pt)
	{
		CRect rc(pt.x, pt.y, pt.x/*+280*/, pt.y/*+190*/);

		Create(hWnd, rc, NULL, WS_POPUP | WS_DLGFRAME | WS_THICKFRAME/* | WS_VISIBLE*/, 0/*exStyle*/);
		return 0;
	}

	CAvailableCSSPropertiesDlg m_dlg;
...
END_MSG_MAP()
	...
	LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		AdjustWindowRect(&rect, WS_DLGFRAME | WS_THICKFRAME, FALSE);
	...
	LRESULT OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		m_dlg.MoveWindow(0, 0, client.right, client.bottom);
		return 0;
	}
...
int g_nCSSCategories = (sizeof(CSSCategories)/sizeof(WCHAR*));
...
CSSProperty CSSProperties[] =
{
	...
	{L"border-bottom-color",				Cat_Border, FALSE,	L"currentColor", TYPE_COLOR()},
	...
	{L"border-collapse",						Cat_Table, TRUE,	L"separate", TYPE_ENUM(L"collapse|separate")},
	{L"border-color",							Cat_Border, 0,	NULL, },	// shorthand
	{L"border-left",							Cat_Border, 0, NULL, }, // shorthand
	{L"border-left-color",					Cat_Border, FALSE,	L"currentColor", TYPE_COLOR()},
	...
	{L"border-right-color",					Cat_Border, FALSE,	L"currentColor", TYPE_COLOR()},
	...
	{L"border-spacing",						Cat_Table, TRUE,	L"0", TYPE_LENGTH()},
	{L"border-style",							Cat_Border, 0,	NULL, },	// shorthand 
	{L"border-top",							Cat_Border, 0, NULL, }, // shorthand
	{L"border-top-color",					Cat_Border, FALSE,	L"currentColor", TYPE_COLOR()},
	...
	{L"box-shadow",							Cat_Border, FALSE,	L"none"/* TYPE_LENGTH()*/},
	...
	{L"color",									Cat_Color, TRUE,	L"black", TYPE_COLOR() },// ?
	...
	{L"color-profile",						Cat_Color, TRUE, L"auto"},
	...
	{L"empty-cells",							Cat_Table, TRUE,	L"show", TYPE_ENUM(L"show|hide")},
	{L"enable-background",					Cat_SVG, FALSE, L"accumulate"},

	{L"fill",									Cat_SVGFill, TRUE, L"#000000", TYPE_SVGPAINT()},
	...
	{L"font",									Cat_Font, TRUE, L""},
	{L"font-family",							Cat_Font, TRUE, L"Times New Roman"/*, TYPE_STRING()*/},
	{L"font-size",								Cat_Font, TRUE, L"16"},
	{L"font-size-adjust",					Cat_Font, TRUE, L""},
	{L"font-stretch",							Cat_Font, TRUE,  L""},
	{L"font-style",							Cat_Font, TRUE, L"normal", TYPE_ENUM(L"normal|italic|oblique")},
	{L"font-variant",							Cat_Font, TRUE, L""},
	{L"font-weight",							Cat_Font, TRUE, L"normal", TYPE_ENUM(L"normal|bold||100|200|300|400|500|600|700|800|900||bolder|lighter")},
	...
	{L"opacity",								Cat_Color, FALSE, L"1", TYPE_NUMBER(0, 1, 100) },
	...
	{L"rendering-intent",					Cat_Color, TRUE,	L"auto", TYPE_ENUM(L"auto|perceptual|relative-colorimetric|saturation|absolute-colorimetric") },
	...
	{L"stroke",									Cat_SVGStroke, TRUE, L"none", TYPE_SVGPAINT()},
	...
	{L"text-rendering",						Cat_SVGText, TRUE, L"auto", TYPE_ENUM(L"auto|optimizeSpeed|optimizeLegibility|geometricPrecision") },
...
//	m_newProperty.Create(m_hWnd, CRect(0,0,0,0), NULL, BS_BITMAP | WS_CHILD | WS_VISIBLE, 0/*exStyle*/, 1/*id*/);
//	m_newProperty.SetBitmap(LoadBitmap(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDB_BUTTON_NEW)));
//	m_newProperty.EnableWindow(FALSE);
	...
	m_hButtonsBitmap = LoadBitmap(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDB_SMALLBUTTONS));
	m_axbuttonsCtl.Create(m_hWnd, rc, NULL, WS_CHILD | WS_VISIBLE);
	m_axbuttonsCtl.AttachControl(m_buttonsCtl, &p);
	m_buttonsCtl->SetBitmap(m_hButtonsBitmap, 14);
	m_buttonsCtl->AddButton(1, 0);
	m_buttonsCtl->AddButton(2, 1);
	IDispEventImpl<3, CCSSPropertiesDlg, &DIID__IUIToolbarButtonsEvents, &LIBID_UILib, 1, 0>::DispEventAdvise(m_buttonsCtl);
...
//	uiActiveManager->RemoveEventHandler(this);
	...
	IDispEventImpl<3, CCSSPropertiesDlg, &DIID__IUIToolbarButtonsEvents, &LIBID_UILib, 1, 0>::DispEventUnadvise(m_buttonsCtl);
				...
				while (pos)
				{
					...
					dc.FillSolidRect(
						&CRect(itemrect.left, itemrect.bottom-1,
								m_splitx/*itemrect.right*/, itemrect.bottom),
					...
					*/
					...
					else if (pItem->m_type == 2)
					{
						...
						{
							sysstring value = m_style->getPropertyValue(pProperty->m_pType->m_name);
							...
							case 0:
								{
									if (!wcscmp(pProperty->m_pType->m_name, L"clip"))
									{
										//if (!pItem->m_expanded)
										{
											CComPtr<ILCSSPrimitiveValue> cssvalue = m_style->getPropertyCSSValue(pProperty->m_pType->m_name);
									...
									else if (!wcscmp(pProperty->m_pType->m_name, L"font-family"))
									{
										if (!pItem->m_expanded)
										{
											CComPtr<ILCSSValueList> cssvalue = m_style->getPropertyCSSValue(pProperty->m_pType->m_name);
											...
											// Draw first item only
												if (count > 0)
												{
													CComPtr<ILCSSPrimitiveValue> nvalue = cssvalue->item(0);

													sysstring value = nvalue->getStringValue();
									...
									else if (!wcscmp(pProperty->m_pType->m_name, L"font-size"))
									{
										...
										if (TRUE)
										{
											...
											CRect arrowrc = CRect(rc.right-16, rc.top, rc.right, rc.bottom);
											dc.DrawFrameControl(&arrowrc, DFC_SCROLL, DFCS_SCROLLCOMBOBOX);

											trect = CRect(rc.left, rc.top, rc.right-16, rc.bottom);
											trect.InflateRect(-1, -1);
										}
										else
										{
							...
							case CSSType_Uri:
								{
									...
									if (cssvalue)
									{
										sysstring value = cssvalue->getStringValue();
							...
							default:
								{
									...
									dc.DrawEdge(&trect, EDGE_SUNKEN, BF_RECT/*, BF_ADJUST*/);
									trect.InflateRect(-GetSystemMetrics(SM_CXEDGE),-GetSystemMetrics(SM_CYEDGE));
						...
						{
							sysstring value = m_style->getPropertyValue(pProperty->m_name);
					...
					else if (pItem->m_type == 3)	// Value
					{
						...
						if (!wcscmp(pProperty->m_pType->m_name, L"clip"))
						{
							...
							if (cssvalue)
							{
								LCSS::LCSSUnitType unitType = cssvalue->get_primitiveType();

								if (unitType == CSS_RECT)
								{
									CComPtr<ILCSSRect> rect = cssvalue->getRectValue();

									CComPtr<ILCSSPrimitiveValue> csstop = rect->get_top();
									values[0] = csstop->get_cssText();

									CComPtr<ILCSSPrimitiveValue> cssright = rect->get_right();
									values[1] = cssright->get_cssText();

									CComPtr<ILCSSPrimitiveValue> cssbottom = rect->get_bottom();
									values[2] = cssbottom->get_cssText();

									CComPtr<ILCSSPrimitiveValue> cssleft = rect->get_left();
									values[3] = cssleft->get_cssText();
								}
							}
							...
							int y2 = y;
							CRect valuerect(x+1, y2, m_areaRect.Width(), y2+21-1);
						...
						else if (!wcscmp(pProperty->m_pType->m_name, L"font-family"))
						{
							...
							for (int i = 0; i < count; i++)
							{
								...
								CRect valuerect(x+1, y2, m_areaRect.Width(), y2+21);
					...
					else
					{
						if (pFItem->m_type == 2)
						{
							...
							else
							{
								...
								case CSSType_String:
									{
										m_hWndEdit = CreateWindowEx(WS_EX_CLIENTEDGE, "edit", NULL, WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL,
												...
												default:
													{
														...
														m_style->getPropertyCSSValue(pProperty->m_pType->m_name, &cssvalue);
														CComQIPtr<ILSVGColor> svgColor = cssvalue;
														...
														switch (nID)
														{
														case 3:	colorFormat = 0; break;
														case 4:	colorFormat = 1; break;
														case 5:	colorFormat = 2; break;
														case 6:	colorFormat = 3; break;
														}
														
														m_style->setProperty(pProperty->m_pType->m_name, MakeRGBString(colorFormat, red, green, blue, alpha), NULL);
													}
													break;
												}
...
void CCSSPropertiesDlg::OnSize()
{
	...
	*/
	...
	m_axbuttonsCtl.MoveWindow(client.right-16-16*2, m_areaRect.bottom, 16*2, 16);
...
void __stdcall CCSSPropertiesDlg::OnToolbarButtonClick(long buttonid)
{
	if (m_style)
	{
		if (buttonid == 2)
		{
			CPopupDialog* pPopup = new CPopupDialog;

			CPoint pt(0, m_areaRect.bottom+16);
			ClientToScreen(&pt);

			pPopup->m_dlg.m_viewGroup = m_viewGroup;
			pPopup->m_dlg.m_style = m_style;

			pPopup->Popup(m_hWnd, pt);
		}
	}
}
...
void CCSSPropertiesDlg::RebuildList()
{
	if (m_pTree)
	{
		delete m_pTree;
	...
	long count;
	m_style->get_length(&count);
	for (int n = 0; n < count; n++)
	{
		CComBSTR propertyName;
		m_style->item(n, &propertyName);

		int i = GetCSSPropertyIndex(propertyName);
		//if (i >= 0)
		{
			CCSSProperty* pProperty = new CCSSProperty;
			pProperty->m_name = propertyName;//CSSProperties[i].m_name;
			if (i >= 0)
				pProperty->m_pType = &CSSProperties[i];
			else
				pProperty->m_pType = NULL;
			...
			if (pProperty->m_pType)
			{
				...
				if (pProperty->m_pType->d.m_nResource)
				{
					pProperty->m_hImageList = ImageList_Create(18, 18, ILC_COLOR8 | ILC_MASK, 0, 10);

					HBITMAP hBitmap = LoadBitmap((HINSTANCE)_Module.GetResourceInstance(), MAKEINTRESOURCE(pProperty->m_pType->d.m_nResource));
					if (hBitmap)
					{
						ImageList_AddMasked(pProperty->m_hImageList, hBitmap, RGB(192, 192, 192));
...
STDMETHODIMP CCSSPropertiesDlg::handleActivateObjectEvent(IUnknown* object, long* cookie)
{
	...
	if (viewGroup)
	{
		...
		{
			m_axvert.ShowWindow(SW_SHOW);
			m_axbuttonsCtl.ShowWindow(SW_SHOW);
...
STDMETHODIMP CCSSPropertiesDlg::handleDeactivateObjectEvent(IUnknown* object, long cookie, BOOL* bAllow)
{
	...
	{
		m_axvert.ShowWindow(SW_HIDE);
		m_axbuttonsCtl.ShowWindow(SW_HIDE);
