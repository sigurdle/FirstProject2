...
bool XMLParser::load(StringIn xmlSource)
{
	return true;
}
...
void XMLParser::CollectSpaces(String& spaces)
{
	//spaces = OLESTR("");
...
//	StringBuilderW str(&buffer);
	...
	// = buffer.DetachToString();

	if (str2.GetLength() == 0)
	{
		String msg = NULL;//OLESTR("Expected ident");
		handleError(CreateDOMError(SEVERITY_FATAL_ERROR, msg, getcnexturrentLocation()));
		THROW(msg);
	}
...
String XMLParser::GetString()
{
	...
	{
		WCHAR msg[64];
	//	swprintf_s(msg, L"Expected \" or ' at offset %d", m_fpos);
...
//	SkipSpaces();
	...
	while (!eof())
	{
		...
		if (c == L'&')
		{
			...
			*namestream = 0;
			//StringW* name = namebuffer.DetachToString();

			if (c == L';')
			{
				uint16 uc = GetUCode(namebuffer);
...
{
	int startoffset = m_fpos-data.GetLength();
	...
	while (!eof())
	{
		...
		if (c == '&')
		{
			...
			*namestream = 0;
			...
			//WCHAR* t = buffer.DetachToString();
			//t << (WCHAR)0;

		// TODO?, should make sure there's a ';'

			uint32 uc = GetUCode(namebuffer);
			if (uc)
			{
				stream << Utf32Char(uc);
			}
...
void XMLParser::ParseAttributes(Node* parent, NamedNodeMap* attributes)
{
	...
	while (!eof())
	{
		...
		if (attrLocalName.GetLength() == 0)
		{
			String msg = WSTR("Error parsing attributes");
			handleError(CreateDOMError(SEVERITY_FATAL_ERROR, msg, getcnexturrentLocation()));
			THROW(msg);
		}

//		StringW* qualifiedName = attrPrefix.length()? attrPrefix + OLESTR(":") + attrLocalName: attrLocalName;
	...
	// ??
			...
			{
				namespaceURI = NULL;//WSTR("");	// no namespace
			}
			else
			{
				...
				{
					WCHAR msg[256];
				//	swprintf_s(msg, L"Namespace %s not found for attribute", attrPrefix->c_str());
		...
		{
			WCHAR msg[64];
			//swprintf_s(msg, L"Invalid attribute name %s", qualifiedName->c_str());
		...
		*/
		...
		else
		{
			...
			{
				WCHAR msg[64];
				swprintf_s(msg, L"Expected =");

			// However we only throw an error and not a fatalError
				if (!handleError(CreateDOMError(SEVERITY_ERROR, msg, getcnexturrentLocation())))
				{
					THROW(msg);
				}
			}
		...
		if (c == L'<')
		{
			...
			if (c == L'!')
			{
				...
				if (c == L'[')	// CDATA section
				{
					...
					{
						String msg = WSTR("Expected CDATA");
				...
				else	// DOCTYPE or DTD declaration
				{
					...
					{
						WCHAR msg[512];
					//	swprintf_s(msg, L"Unexpected ident %s", id->c_str());
...
//	TRACE("<%S>\n", (WCHAR*)taglocalName);
	...
	if (taglocalName.GetLength() == 0)
	{
		String msg = WSTR("Expected tagname after <");
...
//		TRACE("TODO Add Default attributes\n");
	...
	if (tagprefix/*->Length() > 0*/ && namespaceURI == NULL/*->Length() == 0*/)
	{
		delete attributes;

		WCHAR msg[512];
		//swprintf_s(msg, L"Undefined namespace %s", tagprefix->c_str());
	...
	String xmlspace = element->getAttribute(WSTR("xml:space"));
...
// ??
	...
	if (bEndTag)
	{
		...
		else
		{
			...
			if (endtagprefix/*->Length() > 0*/ && endnamespaceURI == NULL/*->Length() == 0*/)
			{
				WCHAR msg[512];
				//swprintf_s(msg, L"Undefined namespace %s", endtagprefix->c_str());
		...
		{
			WCHAR msg[512];
			//swprintf_s(msg, L"End tag \"%s\" doesn't match \"%s\"", taglocalName->c_str(), endtaglocalName->c_str());
	...
	{
		WCHAR msg[512];
		//swprintf_s(msg, L"Missing endtag for \"%s\"", taglocalName->c_str());
...
void XMLParser::set_document(Document* newVal)
{
	m_document = newVal;
}
...
// <!DOCTYPE has been read
	...
	if (doctype)
	{
		...
		//doctype->m_name = name;//set_name(name);
		...
		{
			String msg = WSTR("Expected PUBLIC or SYSTEM");
...
//	m_document.Release();
	...
	{
		readbyte();
		ASSERT(m_nextByte == 0xBB);

		readbyte();
		ASSERT(m_nextByte == 0xBF);
				...
				if (c == L'!')
				{
					...
					else	// Maybe DOCTYPE
					{
						...
						if (id == L"DOCTYPE")
						{
							...
							{
								String msg = WSTR("Can only have one doctype node");
...
#endif
					{
						m_document = new GenericDocument;//.CoCreateInstance(CLSID_LDOMDocument);
					}
...
#endif
					m_document->set_errorHandler(m_errorHandler);
				//	m_document->set_isXML(m_bXML);

					// hm.. do this here ?
					if (true)
					{
						Style::IDocumentStyle* documentStyle = dynamic_cast<Style::IDocumentStyle*>(m_document);
						if (documentStyle)
						{
#if 0//_WINDOWS
...
//		m_oldc = EOF;
	...
	// Validate document
		...
		//	printf("Successfully Parsed XML document\n");
...
/*
bool XMLParser::load(IGetByte* stream)
{
	if (stream == NULL)
		THROW(-1);
...
bool XMLParser::load(Stream& stream)
{
	/*
	if (stream == NULL)
		THROW(-1);
...
bool XMLParser::load(StringIn xmlSource)
{
	...
	Stream stream = binder->BindToObject(xmlSource, NULL);
...
class StringWStream : public IO::ISequentialByteStream
{
public:
	StringWStream(String str)
	{
		m_pos = 0;
		m_str = str;
	}
	...
	virtual ULONG Read(void* pv, ULONG cb)
	{
		...
		ULONG n = cb;
		if (m_pos + n > m_str.GetLength())
			n = m_str.GetLength() - m_pos;

		for (int i = 0; i < n; i++)
		{
			ASSERT(0);
#if 0
			pb[i] = m_str->c_str()[m_pos++];
#endif
		}
...
//	virtual LONGLONG GetSize() const;
	...
	virtual IO::ISequentialByteStream* Clone()
	{
		ASSERT(0);
		return NULL;
	}
...
//	StringWStream stream(sourceText);
	m_stream = StringStream(sourceText);

//	m_textLen = sourceText->Length();
//	m_textData = sourceText->begin();
...
IDOMErrorHandler* XMLParser::get_errorHandler()
{
	return m_errorHandler;
}

void XMLParser::set_errorHandler(IDOMErrorHandler *newVal)
{
