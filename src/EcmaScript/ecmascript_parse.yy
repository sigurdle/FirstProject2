...
%token <int_literal> INTEGER_LITERAL
%token <float64_literal> FLOAT_LITERAL
%token <id> IDENTIFIER
%token NULL_KW
%token TRUE_KW
%token FALSE_KW
...
%type <int_literal> AssignmentOperator
...
%type <exp> Initialiser
%type <exp> Initialiser_opt
...
NumericLiteral:	INTEGER_LITERAL { $$ = A_Int32Exp($1) }
			|	FLOAT_LITERAL { $$ = new Float64Exp($1) }
/*
	HexIntegerLiteral
...
StringLiteral:	STRING_LITERAL {	$$ = new A_StringExp($1);	}
...
CallExpression:	MemberExpression Arguments		{ $$ = new A_CallExp($1, $2) }
					...
					|	CallExpression '.' Identifier	{ $$ = new A_MemberExp($1, $3) }
;
...
MultiplicativeExpression:	UnaryExpression
						|	MultiplicativeExpression '*' UnaryExpression	{ $$ = A_BinopExp('*', $1, $3) }
						|	MultiplicativeExpression '/' UnaryExpression	{ $$ = A_BinopExp('/', $1, $3) }
;
...
VariableDeclaration:	Identifier Initialiser_opt	{ $$ = new A_VarDecl($1, $2) }
;

Initialiser_opt:	{ $$ = NULL }
					|	Initialiser
;
...
FunctionDeclaration:	FUNCTION_KW Identifier '(' FormalParameterList_opt ')' '{' FunctionBody '}'	{ $$ = new A_Function($2, Reverse($4), $7) }
;

FunctionExpression:	FUNCTION_KW /*Identifier_opt*/ '(' FormalParameterList_opt ')' '{' FunctionBody '}'		{ $$ = new A_Function(nullptr, Reverse($3), $6) }
;
...
FormalParameterList: Identifier	{ $$ = new A_IdentifierList($1, NULL) }
								|	FormalParameterList ',' Identifier	{ $$ = new A_IdentifierList($3, $1) }
;
