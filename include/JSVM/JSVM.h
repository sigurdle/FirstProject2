...
class JSVMEXT EcmaScriptException : public Exception
{
public:
	CTOR EcmaScriptException(StringIn reason, Exception* innerException = nullptr, HRESULT hr = E_FAIL) : Exception(reason, innerException, hr)
	{
	}
};
...
public:
	...
	byte* GetArgs(FunctionType* pFun, byte* args, Variant* v, unsigned int numargs);
