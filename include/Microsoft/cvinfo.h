...
typedef unsigned int  CV_typ_t;	// TODO remove
typedef unsigned short  CV_typ_t16;
typedef unsigned int  CV_typ_t32;
...
#if CC_BIGINT
#define CV_TYP_IS_SIGNED(typ)   (((CV_TYPE(typ) == CV_SIGNED)  && CV_TYP_IS_DIRECT(typ)) || (typ == T_INT1) || (typ == T_INT2) || (typ == T_INT4) || (typ == T_INT8) ||  (typ == T_INT16) || (typ == T_RCHAR))
#define CV_TYP_IS_UNSIGNED(typ) (((CV_TYPE(typ) == CV_UNSIGNED)  && CV_TYP_IS_DIRECT(typ)) ||                                     (typ == T_UINT1) ||                                   (typ == T_UINT2) ||                                    (typ == T_UINT4) ||                                     (typ == T_UINT8) ||                                     (typ == T_UINT16))
#else
#define CV_TYP_IS_SIGNED(typ)   (((CV_TYPE(typ) == CV_SIGNED)  && CV_TYP_IS_DIRECT(typ)) ||                                     (typ == T_INT1) ||                                     (typ == T_INT2) ||                                     (typ == T_INT4) ||                                     (typ == T_RCHAR))

#define CV_TYP_IS_UNSIGNED(typ) (((CV_TYPE(typ) == CV_UNSIGNED)  && CV_TYP_IS_DIRECT(typ)) ||                                  (typ == T_UINT1) ||                                  (typ == T_UINT2) ||                                   (typ == T_UINT4))
#endif
...
#define CV_IS_INTERNAL_PTR(typ) (CV_IS_PRIMITIVE(typ) &&     CV_TYPE(typ) == CV_CVRESERVED &&   CV_TYP_IS_PTR(typ))
...
typedef struct lfModifier_16 {
    unsigned short  leaf;           // LF_MODIFIER
    CV_modifier_t   attr;           // modifier attribute modifier_t
    unsigned short       type;           // modified type
} lfModifier_16;

typedef struct lfModifier_32 {
    unsigned short  leaf;           // LF_MODIFIER_32
    CV_typ_t32        type;           // modified type
    CV_modifier_t   attr;           // modifier attribute modifier_t
} lfModifier_32;
...
{
   unsigned short      leaf;           // LF_POINTER
   CV_typ_t32        index;      // type index if CV_PTR_BASE_TYPE
...
//	unsigned char   name[1];
            unsigned short  pmenum;     // enumeration specifying pm format
            CV_typ_t32        pmclass;    // index of containing class for pointer to member
};
...
typedef struct lfClass_16 {
    unsigned short  leaf;           // LF_CLASS, LF_STRUCT
    unsigned short  count;          // count of number of elements in class
    short        field;          // type index of LF_FIELD descriptor list
    short       property;       // property attribute field (prop_t)
    short        derived;        // type index of derived from list if not zero
    short        vshape;         // type index of vshape table for this class
...
typedef lfClass_16 lfStructure_16;

typedef struct lfClass_32 {
    unsigned short  leaf;           // LF_CLASS, LF_STRUCT
    unsigned short  count;          // count of number of elements in class
    CV_prop_t       property;       // property attribute field (prop_t)
    CV_typ_t32      field;          // type index of LF_FIELD descriptor list
    CV_typ_t32      derived;        // type index of derived from list if not zero
    CV_typ_t32      vshape;         // type index of vshape table for this class
	unsigned short int	structlen;
	unsigned char name[1];
/*    unsigned char   data[CV_ZEROLEN]; // data describing length of structure in
                                    // bytes and name
...
typedef lfClass_32 lfStructure_32;
...
typedef struct lfDerived_16 {
    unsigned short  leaf;           // LF_DERIVED
    unsigned short  count;          // number of arguments
    unsigned short        drvdcls[CV_ZEROLEN];  // type indices of derived classes
} lfDerived_16;

typedef struct lfDerived_32 {
    unsigned short  leaf;           // LF_DERIVED
    unsigned short  count;          // number of arguments
	 unsigned short pad;
...
typedef struct mlMethod_16 {
    CV_fldattr_t   attr;           // method attribute
    unsigned short       index;          // index to type record for procedure
    unsigned long  vbaseoff[CV_ZEROLEN];    // offset in vfunctable if intro virtual
} mlMethod_16;

typedef struct mlMethod_32 {
    CV_fldattr_t   attr;           // method attribute
	 uint16 pad;
...
typedef struct lfBitfield32 {
    unsigned short  leaf;           // LF_BITFIELD
    CV_typ_t32      type;           // type of bitfield
    unsigned char   length;
    unsigned char   position;
} lfBitfield32;
...
typedef struct lfVBClass_16 {
    unsigned short  leaf;           // LF_VBCLASS | LV_IVBCLASS
    unsigned short   index;          // type index of direct virtual base class
    unsigned short   vbptr;          // type index of virtual base pointer
    CV_fldattr_t    attr;           // attribute
...
typedef struct lfVFuncTab_16 {
    unsigned short  leaf;           // LF_VFUNCTAB
    unsigned short        type;           // type index of pointer
} lfVFuncTab_16;

typedef struct lfVFuncTab_32 {
    unsigned short  leaf;           // LF_VFUNCTAB
	 unsigned short pad;
...
typedef struct lfNestType_16 {
    unsigned short  leaf;           // LF_NESTTYPE
    unsigned short  index;          // index of nested type definition
    unsigned char   Name[1];        // length prefixed type name
} lfNestType_16;

typedef struct lfNestType_32 {
    unsigned short  leaf;           // LF_NESTTYPE
	 unsigned short	pad;
...
S_REGISTER_32  = 0x1001, /* Variants with new 32-bit type indices */
S_CONSTANT_32  = 0x1002,
S_UDT_32       = 0x1003,
S_COBOLUDT_32  = 0x1004,
S_MANYREG_32   = 0x1005,
...
typedef struct CONSTSYM16 {
    unsigned short  reclen;         // Record length
    unsigned short  rectyp;         // S_CONSTANT
    CV_typ_t16      typind;         // Type index (containing enum if enumerate)
    unsigned short  value;          // numeric leaf containing value
    unsigned char   name[CV_ZEROLEN];         // Length-prefixed name
} CONSTSYM16;

typedef struct CONSTSYM32 {
    unsigned short  reclen;         // Record length
    unsigned short  rectyp;         // S_CONSTANT
    CV_typ_t32      typind;         // Type index (containing enum if enumerate)
    unsigned short  value;          // numeric leaf containing value
    unsigned char   name[CV_ZEROLEN];         // Length-prefixed name
} CONSTSYM32;

typedef struct UDTSYM16 {
    unsigned short  reclen;         // Record length
    unsigned short  rectyp;         // S_UDT | S_COBOLUDT
    CV_typ_t16      typind;         // Type index
    unsigned char   name[1];        // Length-prefixed name
} UDTSYM16;

typedef struct UDTSYM32 {
    unsigned short  reclen;         // Record length
    unsigned short  rectyp;         // S_UDT | S_COBOLUDT
    CV_typ_t32      typind;         // Type index
    unsigned char   name[1];        // Length-prefixed name
} UDTSYM32;
