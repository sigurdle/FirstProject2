#ifndef Graphics_wmf_h
#define Graphics_wmf_h
...
enum WmfFunction : unsigned short
{
	wmfAbortDoc  = 0x0052 ,
	wmfArc  = 0x0817 ,
	wmfChord  = 0x0830, 
...
class GfxExt WindowsMetafileException : public Exception
{
public:
	CTOR WindowsMetafileException(StringIn str) : Exception(str)
	{
	}
};
...
class GfxExt LoaderBase : public Object
{
public:
	virtual ~LoaderBase()
	{
...
class GfxExt Loader2 : public LoaderBase
{
public:
	CTOR Loader2();

	void Load(IO::Stream* pStream);

	void GetRect(gm::RectD* prect) const
	{
		//*prect = m_rect;
		*prect = gm::RectD(m_rect.X, m_rect.Y, m_rect.Width, m_rect.Height);
	}

	gm::RectF GetRect() const
	{
		return m_rect;
	}

	virtual void FileType() abstract;

	virtual void SetMapMode(int mapmode) abstract;
	virtual void SetWindowOrg(int x, int y) abstract;
	virtual void SetWindowExt(int cx, int cy) abstract;
	virtual void SetViewportOrg(int x, int y) abstract;
	virtual void SetViewportExt(int cx, int cy) abstract;
	virtual void SetTextColor(COLORREF color) abstract;
	virtual void SetTextAlign(uint32 textAlign) abstract;
	virtual void SetMiterLimit(float miterLimit) abstract;
	virtual void MoveTo(int x, int y) abstract;
	virtual void ExtTextOut(int x, int y, StringIn string, const int* distances) abstract;
	virtual void SelectObject(int n) abstract;
	virtual void DeleteObject(int n) abstract;
	virtual void CreatePenIndirect(int index, const Graphics::LOGPEN& lp) abstract;
	virtual void CreatePenIndirect(int index, const Graphics::EXTLOGPEN& lp) abstract;
	virtual void CreateBrushIndirect(int index, const Graphics::LOGBRUSH& lb) abstract;
	virtual void CreateFontIndirect(int index, const Graphics::LOGFONT_16& lf) abstract;
	virtual void CreateFontIndirect(int index, const Graphics::LOGFONTA& lf) abstract;
	virtual void CreateFontIndirect(int index, const Graphics::LOGFONTW& lf) abstract;
	virtual void StretchDIBits(int destX, int destY, int destWidth, int destHeight, int srcX, int srcY, int srcWidth, int srcHeight, Graphics::Bitmap* bitmap) abstract;
	virtual void FillPath() abstract;

	Bitmap* ReadDib(IO::Stream* pStream);
	gm::PointF LPToDP(int Lx, int Ly);

	int m_numOfObjects;	// TODO
	wmfObject** m_objects;	// TODO
