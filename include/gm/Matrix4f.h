// matrix4x4f.h - 4x4 element matrix class
//
// libgm++: Graphics Math Library
// Ferdi Scheepers and Stephen F May
// 15 June 1994

#ifndef gmmatrix4x4f_H
#define gmmatrix4x4f_H
...
public:
	...
	CTOR matrix4x4f(const matrix4x4f& M)
	{
		m_[0][0] = M[0][0];
		m_[0][1] = M[0][1];
		m_[0][2] = M[0][2];
	...
	CTOR matrix4x4f(const matrix3f& other);

	CTOR matrix4x4f(const float*);

	CTOR matrix4x4f(float, float, float, float,
	...
	matrix4x4f& assign(float, float, float, float,
		    float, float, float, float,
			 float, float, float, float,
			 float, float, float, float);
	...
	friend matrix4x4f operator *(float, const matrix4x4f&);
	
	bool operator ==(const matrix4x4f&) const;
	bool operator !=(const matrix4x4f&) const;

	bool FuzzyEquals(const matrix4x4f&) const;
	
	vector4f operator *(const vector4f&) const;
	
	friend LFCEXT vector4f operator *(const vector4f&, const matrix4x4f&);
	
	// operations
	
	matrix4x4f getInverse() const;
	matrix4x4f getTranspose() const;
	matrix4x4f getAdjoint() const;
	...
	// gmVector3t<float> matrix4x4f::transform_transpose(const gmVector3t<float>& v) const;
	...
	static matrix4x4f getRotate(float, const vector3f& axis);
	static matrix4x4f getScale(float, float, float);
	static matrix4x4f getTranslate(float, float, float);
	static matrix4x4f getRotate(float, float x, float y, float z);
	static matrix4x4f getScale(const vector3f&);
	static matrix4x4f getTranslate(const vector3f&);
	
	static matrix4x4f getPerspective(double fovy, double aspect, double zNear, double zFar);
	static matrix4x4f getOrtho(double left, double right, double bottom, double top, double zNear, double zFar);
	static matrix4x4f getOrtho2D(double left, double right, double bottom, double top);
	
	// cubic basis matrices
	
	static matrix4x4f bezierBasis();
	static matrix4x4f bsplineBasis();
	static matrix4x4f catmullromBasis();
	static matrix4x4f hermiteBasis();
	
	static matrix4x4f tensedBSplineBasis(float);
	static matrix4x4f cardinalBasis(float);
	static matrix4x4f tauBasis(float, float);
	static matrix4x4f betaSplineBasis(float, float);
...
inline matrix4x4f::operator float* ()
{
	return &m_[0][0];
}

inline matrix4x4f::operator const float* () const
{
	return &m_[0][0];
}

inline float* matrix4x4f::operator [](int i)
{
	ASSERT(i == 0 || i == 1 || i == 2 || i == 3);
	return &m_[i][0];
}

inline const float* matrix4x4f::operator [](int i) const
{
	ASSERT(i == 0 || i == 1 || i == 2 || i == 3);
	return &m_[i][0];
}

inline void matrix4x4f::copyTo(float f[4][4]) const
{
...
public:
	CTOR Matrix4();
	CTOR Matrix4(const matrix4x4f& m);
...
//	friend class Matrix4;
	...
	__DECLSPEC_ALIGN_16__ matrix4x4f m_matrix;
};
