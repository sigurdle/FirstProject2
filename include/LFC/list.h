...
public:
	...
	//	typedef list_node Node;
		...
		ptrdiff_t operator - (const const_list_iterator& other) const
		{
			ptrdiff_t diff = 0;
			const list_node* p = m_ptr;
			while (p != m_ptr)
			{
				if (p == other.m_ptr) return diff;
...
*/
	...
	{
		m_head = (node*)m_allocator.allocate_object(sizeof(node));
	...
	*/
	...
	{
		node* n = (node*)m_allocator.allocate_object(sizeof(node));
