...
public:
	...
	CTOR red_black_node_base() : left(nullptr), right(nullptr)
	{
	}
...
public:
	...
	CTOR red_black_node(const pair<key_t, value_t>& keyval) : item(keyval), left(nullptr), right(nullptr)
	{
	}
...
//	~rb_tree_base();
	...
	*/
	...
	//	If a node is red, then both its children are black
		if (node->color == Red)
		{
			VERIFY(node->left == nullptr || node->left->color == Black);
			VERIFY(node->right == nullptr || node->right->color == Black);
		}
...
public:
	...
	Nodeptr newnode()
	{
		Node* x = (Node*)m_allocator.allocate_object(sizeof(*x));
		return x;
	}
	...
	{
	public:
		CTOR const_iterator() : m_p(nullptr)
		{
		}
...
//		validate(head->parent);
		...
		*/
		...
		else if (cur->left == nullptr || cur->right == nullptr)	// one child
		{
		...
		//	MessageBeep(-1);
			...
			ASSERT(succ->left == nullptr);
		//	ASSERT(succ->right == nullptr);
			...
			while (1)
			{
				...
				if (token && token == token->parent->left)
				{
					...
					else
					{
						...
						if (far_nephew == nullptr || far_nephew->color == Black)
						{
		...
		if (cur->left == nullptr || cur->right == nullptr)	// Has max one child
		{
			if (cur->color == Red)
			{
			...
			else if (cur->color == Black && cur->left == nullptr && cur->right == nullptr) // Current is black and has no children
			{
			...
			ASSERT(0);
#endif
		return base::iterator(nullptr);	// TODO
	}
